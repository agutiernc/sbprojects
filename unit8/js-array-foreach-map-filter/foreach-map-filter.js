...
function showFirstAndLast(arr){
	...
	arr.forEach(x => newArr.push(x[0] + x.slice(-1)))
...
function addKeyAndValue(arr, key, value){
	...
	arr.forEach(x => newArr.push(Object.assign(x, { [key]: value })))
...
function find(arr, searchValue) {
	return arr.filter(x => x === searchValue ? x : undefined)[0]
}
...
function removeVowels(str) {
	...
	return arr.filter(x => vowels.indexOf(x) === -1).join('')
}
